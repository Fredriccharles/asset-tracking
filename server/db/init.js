...
function runMigrations() {
  ...
  // 'user' accounts can be saved. Foreign keys are deferred around the rebuild
  // so no data is lost (users are referenced by several tables with SET NULL).
  try {
    ...
    if (check && check.sql && !/role IN \('admin','user'\)/.test(check.sql)) {
      ...
      db.exec(`
        ...
        INSERT INTO users (id, username, password_hash, full_name, role, is_active, last_login_at, created_at, updated_at)
          SELECT id, username, password_hash, full_name, role, is_active, last_login_at, created_at, updated_at FROM users_old;
